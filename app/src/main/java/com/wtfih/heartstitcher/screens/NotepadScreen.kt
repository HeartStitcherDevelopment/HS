...
@Composable
fun NotepadScreen(dataViewModel: UserDataViewModel = viewModel()) {
    ...
    val notes = remember { dataViewModel.state.value["notes"] as? MutableList<Pair<String,Pair<String,String>>> ?: mutableListOf() }
    Box(
        modifier = Modifier
            .fillMaxSize()
            .padding(28.dp)
        ...
        Column(
            ...
            NormalTextComponent(value = stringResource(id = R.string.notepad_info))
            ...
            NormalTextComponent(value = stringResource(id = R.string.new_note))
            ...
            ButtonComponent(
                ...
                onButtonClicked = {
                    notes.add(Pair(dateString, Pair(titleValue, textValue)))
                    db.collection("users").document(id).update("notes", notes)
                    Toast.makeText(
                        context,
                        "Success!",
                        Toast.LENGTH_LONG
            ...
            ButtonComponent(
                value = stringResource(id = R.string.browse),
                onButtonClicked = { HeartStitcherRouter.navigateTo(Screen.BrowseNotesScreen) }
            )
        }
